...
export interface CurrentCrop extends CropEntry {
...
export const BENGALI_MONTHS: string[] = [
  'পৌষ',       // ~Dec 15 - Jan 14
  'মাঘ',       // ~Jan 15 - Feb 13
  'ফাল্গুন',   // ~Feb 14 - Mar 14
  'চৈত্র',     // ~Mar 15 - Apr 13
  'বৈশাখ',    // ~Apr 14 - May 14
  'জ্যৈষ্ঠ',   // ~May 15 - Jun 14
  'আষাঢ়',     // ~Jun 15 - Jul 15
  'শ্রাবণ',    // ~Jul 16 - Aug 15
  'ভাদ্র',     // ~Aug 16 - Sep 15
  'আশ্বিন',    // ~Sep 16 - Oct 15
  'কার্তিক',   // ~Oct 16 - Nov 14
  'অগ্রহায়ণ',  // ~Nov 15 - Dec 14
];
 ...
 */
export function getCurrentCrops(): CurrentCrop[] {
    ...
    crop.seasons.forEach(season => {
      if (season.months.includes(currentMonth)) {
        const riskMonths = season.riskPeriod
          ? season.riskPeriod.split('-').map(m => MONTH_NAME_MAP[m.trim()])
          : [];
...
/**
 * Get crops that are in planting season for the current month.
 */
export function getPlantingCrops(): CurrentCrop[] {
  const currentMonth = new Date().getMonth() + 1;
  return CROP_CALENDAR.filter(crop =>
    crop.seasons.some(season => season.months.includes(currentMonth))
  ).map(crop => {
